...
import 'package:iron_mine/presentation/screens/quality_users/widgets/quality_storage_ticket_item.dart';
import 'package:iron_mine/presentation/screens/widgets/custom_loading.dart';
import 'package:iron_mine/presentation/viewModel/OrdersViewModel/storage_quality_controller.dart';
...
class QualityStorageTicketScreen extends ConsumerWidget {
  QualityStorageTicketScreen({super.key});

  static StateProvider<bool> rebuildProvider =
      StateProvider<bool>((ref) => false);
  
  // Set default order status based on user role
  // If user is storage quality, show pending orders by default
  ...
  final orderStatus = StateProvider<String>((ref) {
    ...
    return userRole == RolesEnum.storageQuality
...
void handleConnectivityChanges(
    ...
    StorageQualityProvider orderPro = ref.read(storageQualityPro);
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            /// orders status - only shown for storage quality role
            Consumer(
              builder: (context, ref, child) {
                final userRole = TokenUtil.getRoleFromMemory();
                log('Current user role for order status view: $userRole');
                
                // Only show the OrdersTypeList for storage quality users
                if (userRole == RolesEnum.storageQuality) {
                  return Padding(
              ...
              child: Consumer(
                builder: (context, ref, widget) {
                  ...
                  return FutureBuilder<GetStorageQualityTicketsResponse>(
                    future: TicketsRepository.getStorageQualityTickets(
                      getOrderRequest: GetOrdersRequest(limit: '1000'),
                      ticketsType: ticketsType,
                    ),
                    builder: (BuildContext context, AsyncSnapshot<GetStorageQualityTicketsResponse> snapshot) {
                      switch (snapshot.connectionState) {
                        ...
                        default:
                          if ((snapshot.data == null)) {
                            ...
                            List<StorageQualityTicketModel> ticketsList = snapshot.data?.ticketsList ?? [];
                            return ticketsList.isEmpty
                                ...
                                : ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      return QualityStorageTicketItem(
                                        index: index,
