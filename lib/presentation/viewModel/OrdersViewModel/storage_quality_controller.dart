...
ChangeNotifierProvider<StorageQualityProvider> storageQualityPro =
    ChangeNotifierProvider<StorageQualityProvider>(
        (ref) => StorageQualityProvider());

class StorageQualityProvider extends ChangeNotifier {
  bool _sampleReportLoading;
  bool _finalReportLoading;
  bool _isRetrying = false;
  bool _isStatusChanging = false;

  StorageQualityProvider()
  ...
  Future<void> saveReportOffline({
    ...
    final reports = prefs.getStringList('storage_quality_reports') ?? [];
    reports.add(json.encode(reportData));
    await prefs.setStringList('storage_quality_reports', reports);
  }
            ...
            reportType == 'sample'
                ? Consumer(
                    builder: (context, ref, _) {
                      StorageQualityProvider orderPro =
                          ref.watch(storageQualityPro);
  ...
  createSampleReport({
    ...
    TicketsRepository.createStorageQualitySampleReport(
      ticketId: ticketId,
      parametersJson: parametersJson,
      photo: photo,
    ).then(
      (value) {
        ...
        if ((value.id?.isNotEmpty ?? false) && context.mounted) {
          ref.refresh(StorageTicketScreen.rebuildProvider.notifier).state =
              true;
          CustomNavigator.popScreen(context: context);
          ViewFunctions.showCustomSnackBar(
            context: context,
  ...
  createFinalReport({
    ...
    TicketsRepository.createStorageQualitySampleReport(
      ...
      TicketsRepository.createStorageQualityFinalReport(
        ticketId: ticketId,
      ).then(
        (value) {
          _finalReportLoading = false;
          ...
          if ((value.message == null) && context.mounted) {
            ref
                .refresh(QualityStorageTicketScreen.rebuildProvider.notifier)
                .state = true;
            CustomNavigator.popScreen(context: context);
            ViewFunctions.showCustomSnackBar(
              context: context,
  ...
  Future<void> retryOfflineReports(BuildContext context) async {
    ...
    try {
      ...
      await prefs.setStringList('storage_quality_reports', remainingReports);
    ...
    while (retryCount < maxRetries) {
      try {
        if (reportType == 'sample') {
          ...
          final result =
              await TicketsRepository.createStorageQualitySampleReport(
            ticketId: ticketId,
            parametersJson: parametersJson,
          ...
          final result =
              await TicketsRepository.createStorageQualityFinalReport(
            ticketId: ticketId,
          );
          return _handleResponse(context, result);
        }
  ...
  Future<void> _cleanupOldReports() async {
    try {
      ...
      if (validReports.length != reports.length) {
        await prefs.setStringList('storage_quality_reports', validReports);
  ...
  changeTicketStatus({
    ...
    try {
      ...
      final result = await TicketsRepository.changeStorageQualityStatus(
        ticketId: ticketId,
        newStatus: newStatus,
      );
      ...
      if (result.message == null) {
        ...
        ref.refresh(QualityStorageTicketScreen.rebuildProvider.notifier).state =
            true;
      }
    } catch (e) {
      ViewFunctions.showCustomSnackBar(
