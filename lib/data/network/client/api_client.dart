...
class ApiClient {
  static Map<String, String> headers({Map<String, String>? headerType}) {
    var mHeaders = headerType ?? {
       HttpHeaders.acceptHeader: 'application/json',
    };
    mHeaders[HttpHeaders.contentLanguageHeader] =
  ...
  static Future<http.Response> postRequest(
    ...
    //network logger
    log("$url\n${headers(headerType: headersValues)}");
    ...
    if (!isMultipart) {
      response = await http.post(
        Uri.parse(url),
        headers: headers(headerType: headersValues),
        body: requestBody,
      );
    } else {
      ...
      log('message  >>>>> ${multiPartValues![0].contentType.toString()}');
      var request = http.MultipartRequest('POST', uri)
        ..headers.addAll(headers(headerType: headersValues))
